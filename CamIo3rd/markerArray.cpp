...
bool Marker3dGroup::detect(const MarkerArray &markers, const Mat &camMatrix, const Mat &distCoeffs){
    convexHull.clear();
    detectedIds.clear();
    for (int i=0; i<vMarker3d.size(); i++) {
        const vector<Point2f> *corners = markers.getCorner(vMarker3d[i].id_);
...
//        cout<<"1: vMarker3d[i].id_ = "<<vMarker3d[i].id_<<endl;
        const vector<Point2f> *corners = markers.getCorner(vMarker3d[i].id_);
        if (corners==0) continue;
//        cout<<"2: vMarker3d[i].id_ = "<<vMarker3d[i].id_<<endl;

        imagePoints.insert(imagePoints.end(), corners->begin(),corners->end());
        objectPoints.insert(objectPoints.end(), vMarker3d[i].vP3f.begin(), vMarker3d[i].vP3f.end());
    }
    
    return Marker3dGroup::detect(imagePoints, objectPoints,camMatrix, distCoeffs);
}


bool Marker3dGroup::detect(vector<Point2f> &imagePoints, vector<Point3f> &objectPoints,const Mat &camMatrix, const Mat &distCoeffs){
    if (imagePoints.size()<8) return false;  //at least there are 2 markers
    if (_DEBUG) cout<<"imagePoints.size() = "<<imagePoints.size()<<endl;
        
    medianDist = Marker3dGroup::getMedianDist(imagePoints);
    
//    solvePnP(objectPoints, imagePoints, camMatrix, distCoeffs, rvec, tvec,false,CV_DLS); // DLS to get initial guess of rvec,tvec
//    solvePnP(objectPoints, imagePoints, camMatrix, distCoeffs, rvec, tvec,true,CV_ITERATIVE);  // iterative for accuracy
...
//    cout<<"imagePoints.size() = "<<imagePoints.size()<<", "<<"errMax = "<<errMax<<"iMax = "<<iMax<<endl;
//    cout<<"imagePoints[iMax] = "<<imagePoints[iMax]<<endl;
//    cout<<"projectedPoints[iMax] = "<<projectedPoints[iMax]<<endl;
//    cout<<"tvec = "<<tvec<<endl;
    errMean /= imagePoints.size();
    if (_DEBUG) cout<<"errMean, errMax = "<<errMean<<", "<<errMax<<endl;
    if (errMean>10.0 || errMax>30.0) {
